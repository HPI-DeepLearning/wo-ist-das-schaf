    ...
    def flow(self, image):
        ...
        for y in range(height):
            for x in range(width):
                tasks.apply_convolution_2d_kernel(image, buffer_image, y + kernel_radius, x + kernel_radius, self.kernel)
    ...
    def flow(self, image):
        ...
        for y in range(height - 1):
            for x in range(width - 1):
                    tasks.apply_convolution_1d_kernel(image, tmp_buffer, (y + kernel_radius, x + kernel_radius), 0, self.kernel)

        for y in range(height - 1):
            for x in range(width - 1):
                    tasks.apply_convolution_1d_kernel(tmp_buffer, buffer_image, (y + kernel_radius, x + kernel_radius), 1, self.kernel)
